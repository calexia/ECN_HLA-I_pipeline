...
# Remaining questions
# Also a command parameter ?
allelesList = ["HLA-A02:01", "HLA-A03:01", "HLA-B40:01", "HLA-B49:01", "HLA-C03:04", "HLA-C07:01", "HLA-E01:01"]

command_arguments = sys.argv[1:]
if (len(command_arguments) == 3):
    BASAL_FILE_PATH = command_arguments[0]
    IFNA_FILE_PATH = command_arguments[1]
    OUTPUT_FOLDER_PATH = command_arguments[2]
else:
    print('Missing arguments : python3 src/IFNa/HLA-I_IFNa_MAIN_NatureComm.py <basal_file_path> <ifna_file_path> <output_folder_path>')
    exit(0)

DATA_TYPES = [("Basal", BASAL_FILE_PATH), ("IFNa", IFNA_FILE_PATH)]
...
def concatAndClean(allFormattedData):
    dfBasal = allFormattedData[0]
    dfIFNa = allFormattedData[1]

    finalTable = concat.concatResults(dfBasal, dfIFNa)
...
for type in DATA_TYPES:
    ...
    dataFile = pd.read_csv(CURRENT_TYPE_FILE,
                           sep=',',
                           dtype = {
                               "Peptide" : "str",
                               "Length" : np.int64,
    ...
    data = MHCPredictionGeneration(data, OUTPUT_FOLDER_PATH, CURRENT_TYPE, allelesList)
    #call of function to add peptide occurence AND gene annotation
    data = occAnnot.countOccurences(data, CURRENT_TYPE)
    ...
    # dataFilteredOut.to_csv(path_or_buf=OUTPUT_FOLDER_PATH + '{}curated_out_peptides.txt'.format(CURRENT_TYPE), index=False, sep='\t')


"""PIPELINE : CONCATENATION OF RESULTS"""
#calls the function to concat and clean the df
finalTable = concatAndClean(allFormattedData)
# counts the AA occurences of all peptides and saves it into a specific file
